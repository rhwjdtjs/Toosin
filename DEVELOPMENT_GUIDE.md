...
3.  `base: dev` <- `compare: feature/...` 인지 확인하고 PR 생성.
4.  (혼자니까) 본인이 **Merge pull request** 버튼을 눌러 합칩니다.
5.  Desktop으로 돌아와서 **Current Branch**를 `dev`로 바꾸고 **Pull origin** 하여 최신화합니다.
6.  다 쓴 `feature` 브랜치는 **Branch -> Delete**로 삭제하여 정리합니다.
...
*   **큰 바이너리 파일**: 100MB 넘는 에셋은 가급적 커밋하지 않거나 LFS 설정을 확인하세요.
*   **퇴근(작업 종료) 전**: 항상 작업한 내용을 Push 해두세요. (백업 효과)
