...
ATSCharacter::ATSCharacter()
{
	...
	CurrentWeaponType = ETSWeaponType::OneHanded; //초기 무기 타입 설정
}
...
{
	CurrentWeaponType = NewType; //무기 타입 변경
}
void ATSCharacter::EquipWeapon(ATSWeapon* NewWeapon)
{
	if (NewWeapon)
	{
		CurrentWeapon = NewWeapon; //현재 무기 설정
		CurrentWeaponType = NewWeapon->GetWeaponType(); //무기 타입 설정
		// 소켓에 부착 (RightHandSocket 가정)
		FName SocketName = FName("Sword"); // 필요시 변경
		...
		NewWeapon->SetOwner(this); //소유자 설정
	}
}
