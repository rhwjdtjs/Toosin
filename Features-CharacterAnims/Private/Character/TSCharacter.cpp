...
ATSCharacter::ATSCharacter()
{
	...
	FollowCamera->bUsePawnControlRotation = false; //카메라 자체 회전은 사용하지 않음
	// 컨트롤러 회전 설정
...
void ATSCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	{
		if (MoveAction) EnhancedInputComponent->BindAction(MoveAction, ETriggerEvent::Triggered, this, &ATSCharacter::Move); //이동 입력 바인딩
		if (LookAction) EnhancedInputComponent->BindAction(LookAction, ETriggerEvent::Triggered, this, &ATSCharacter::Look); //시점 입력 바인딩
		if (LightAttackAction) EnhancedInputComponent->BindAction(LightAttackAction, ETriggerEvent::Started, this, &ATSCharacter::LightAttack); //경공격 입력 바인딩
		if (HeavyAttackAction) EnhancedInputComponent->BindAction(HeavyAttackAction, ETriggerEvent::Started, this, &ATSCharacter::HeavyAttack); //중공격 입력 바인딩
		if (DodgeAction) EnhancedInputComponent->BindAction(DodgeAction, ETriggerEvent::Started, this, &ATSCharacter::Dodge); //회피 입력 바인딩
		if (GuardAction) // 방어 입력 바인딩
		{
			EnhancedInputComponent->BindAction(GuardAction, ETriggerEvent::Started, this, &ATSCharacter::GuardStart); //가드시작
			EnhancedInputComponent->BindAction(GuardAction, ETriggerEvent::Completed, this, &ATSCharacter::GuardEnd); //가드종료
		}
...
{
	if (CurrentState == ETSCharacterState::Attacking || CurrentState == ETSCharacterState::Stunned || CurrentState == ETSCharacterState::Dead) // 공격 중, 기절 중, 사망 상태에서는 이동 불가
		return; // 함수 종료
...
{
	if (CurrentState == ETSCharacterState::Idle || CurrentState == ETSCharacterState::Moving) // 대기 중, 이동 중
	{
		SetCharacterState(ETSCharacterState::Blocking); //캐릭터	상태를 방어로 설정
		GetCharacterMovement()->MaxWalkSpeed = WalkSpeed * 0.5f; //  최대 걷기 속도 절반으로 설정
		UE_LOG(LogTemp, Warning, TEXT("Guard Start")); //가드 시작
	}
}

void ATSCharacter::GuardEnd()
{
	if (CurrentState == ETSCharacterState::Blocking) // 방어 중
	{
		SetCharacterState(ETSCharacterState::Idle); //캐릭터 상태를 대기로 설정
		GetCharacterMovement()->MaxWalkSpeed = WalkSpeed; //최대 걷기 속도 원래대로 설정
		UE_LOG(LogTemp, Warning, TEXT("Guard End")); //가드 종료
	}
}
...
float ATSCharacter::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	if (CurrentState == ETSCharacterState::Dead) return 0.0f; // 이미 사망한 경우 데미지 무시

	if (CurrentState == ETSCharacterState::Blocking) // 방어 중인 경우
	{
		UE_LOG(LogTemp, Warning, TEXT("Blocked Damage!")); //데미지 차단!
		CurrentStamina -= DamageAmount * 0.5f; // 스태미나 감소
		return 0.0f;
	}
