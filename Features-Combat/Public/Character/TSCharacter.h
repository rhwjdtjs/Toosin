...
public:
  ...
  virtual void SetupPlayerInputComponent(
      class UInputComponent *PlayerInputComponent) override; // 입력 설정 함수
  virtual float TakeDamage(float DamageAmount,
                           struct FDamageEvent const &DamageEvent,
                           class AController *EventInstigator,
                           AActor *DamageCauser) override; // 데미지 처리 함수
  void InitializeStats(); // 데이터 테이블에서 스탯 초기화 함수

  UFUNCTION(BlueprintCallable, Category = "Toosin|State")
  FORCEINLINE ETSCharacterState GetCharacterState() const {
    return CurrentState;
  } // 현재 상태 반환 함수
  UFUNCTION(BlueprintCallable, Category = "Toosin|State")
  FORCEINLINE ETSWeaponType GetWeaponType() const { return CurrentWeaponType; }
  ...
  UFUNCTION(BlueprintCallable, Category = "Toosin|Weapon")
  void EquipWeapon(class ATSWeapon *NewWeapon); // 무기 장착 함수
  FORCEINLINE class ATSWeapon *GetCurrentWeapon() const {
...
public:
  // Getter for CombatComponent
  FORCEINLINE class UTSCombatComponent *GetCombatComponent() const {
