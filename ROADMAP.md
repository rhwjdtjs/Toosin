...
- [ ] **스탯 시스템 확장**: `FTSCharacterStats`에 방어력(Defense), 기본 공격력(BaseAttackPower) 추가.
- [ ] **데미지 공식 구현**: 무기별 데미지 + 공격유형별 배율(경공격 1.0x / 강공격 1.8x / 콤보 보너스).
- [ ] **피격 몽타주 시스템**: 캐릭터별로 다른 피격 몽타주 적용.
- [ ] **가드 판정 세분화**: 무기-무기 충돌(가드 성공, 가해자 무기 튕김) vs 몸 피격(가드 실패, 일반 피격 처리).
- [ ] **가드 에임 오프셋**: 가드 시 공격 방향에 따라 무기 위치 조절.
- [ ] **라운드 기반 학습 환경**: `ATSArenaGameMode` 구현 (라운드 시작/종료/리셋), 아레나 맵 구축.

#### 🧠 AI 기초 (AI Foundation) - `Due: 03.15`

- [ ] **AIController & Behavior Tree**: `ATSEnemyController` 생성, 블랙보드 설정, 라운드 시작 시 BT 자동 실행.
- [ ] **거리 기반 포지셔닝**: 최적 공격 거리 유지 (전진/후퇴/측면 이동).
- [ ] **기본 공격 로직**: 경공격/강공격 랜덤 선택 + 쿨타임 관리.
- [ ] **기본 방어 트리거**: 플레이어 공격 감지 시 가드 진입/해제.
